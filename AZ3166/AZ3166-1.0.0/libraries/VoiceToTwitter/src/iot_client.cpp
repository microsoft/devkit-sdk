...
void _destructor_response(Http_Response *response);
/// Utility methods end
...
int iot_client_blob_upload_step1(const char *blobName)
{
    ...
    Http_Response *response = blobRequest.send();
    bool error = false;
    if (response == NULL)
    {
...
int iot_client_blob_upload_step2(const char *content, int length)
{
    ...
    Http_Response *response = uploadRequest.send(content, length);
    if (response == NULL)
    {
...
int iot_client_blob_upload_step3(bool isSuccess)
{
    ...
    Http_Response *response = notificationRequest.send(temp2, postBodyLength);
    if (response == NULL)
    {
...
int iot_client_send_event(const char *event, int length)
{
    ...
    Http_Response *response = request.send(event, length);
    if (response == NULL)
    {
...
int complete_c2d_message(char *etag)
{
    ...
    Http_Response *response = request.send();
    if (response == NULL)
    {
...
const char *iot_client_get_c2d_message()
{
    ...
    // don't free body since it is returned
    response->body = NULL;
    _destructor_response(response);
    return res;
}
