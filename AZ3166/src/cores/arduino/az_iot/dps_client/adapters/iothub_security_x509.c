...
SECURITY_DEVICE_HANDLE iothub_security_x509_create()
{
    ...
    else
    {
        ...
        DPS_SECURE_DEVICE_HANDLE riot_handle = secure_device_riot_create();
        if (riot_handle == NULL)
        {
            /* Codes_SRS_IOTHUB_SECURITY_x509_07_006: [ If any failure is encountered iothub_security_x509_create shall return NULL ] */
        ...
        else
        {
            ...
            if ((result->x509_certificates = secure_device_riot_get_certificate(riot_handle)) == NULL)
            {
                /* Codes_SRS_IOTHUB_SECURITY_x509_07_006: [ If any failure is encountered iothub_security_x509_create shall return NULL ] */
            ...
            else if ((result->x509_private_key = secure_device_riot_get_alias_key(riot_handle)) == NULL)
            {
                /* Codes_SRS_IOTHUB_SECURITY_x509_07_006: [ If any failure is encountered iothub_security_x509_create shall return NULL ] */
            ...
            secure_device_riot_destroy(riot_handle);
        }
    }
    return result;
}
